...
def fit_retention_curve(days, rates):
    ...
    try:
        # 使用 scipy 进行非线性拟合，限制 a 在 0-1 之间
...
with col2:
    ...
    if a is not None:
        ...
        fig.add_trace(go.Scatter(
            ...
            fill='tozeroy',
            # 优化点：hovertemplate 强制显示为无小数的整数，且加千分位分隔符
        ...
        # 优化点：LT 值保留 2 位小数，增强可读性
        st.success(f"前 {lt_n} 天的累计留存价值 (LT) 为: {lt_val:.2f} 天")
    else:
        st.warning("请确保已输入至少两个不同的留存观测点以进行拟合。")
